...
function CompactStat({
  ...
  return (
    <article className={`rounded-xl p-3.5 ring-1 ring-inset sm:p-4 ${styles.bg} ${styles.ring}`}>
      <div className="flex items-start gap-2.5">
        <span
          className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/80 shadow-sm ${styles.icon}`}
          aria-hidden
        >
          <Icon className="h-4 w-4" strokeWidth={2.25} />
        </span>
        <div className="min-w-0 flex-1">
          <p className={`font-heading text-xl font-bold leading-none tracking-tight tabular-nums sm:text-2xl ${styles.value}`}>
            {stat.value}
          </p>
...
function FeaturedStat({ stat }: { stat: StatItem }) {
  ...
  return (
    ...
    >
      ...
      <p className="relative mt-2 font-heading text-base font-bold leading-snug text-brand-ink sm:text-lg">
        {stat.label}
      </p>
      {stat.hint ? (
        <p className="relative mt-1.5 text-xs text-brand-muted sm:text-sm">{stat.hint}</p>
      ) : null}
    </article>
  );
}
...
function SponsorshipPanel({ withSponsor, withoutSponsor }: { withSponsor: StatItem; withoutSponsor: StatItem }) {
  ...
  return (
    <article className="rounded-xl bg-brand-warm/80 p-4 ring-1 ring-inset ring-brand-border/60 sm:p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between md:gap-6">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2.5">
            <span
              className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white shadow-sm text-brand-success"
              aria-hidden
            >
          ...
          <div className="mt-4 md:mt-4">
            <div
              className="flex h-2.5 overflow-hidden rounded-full bg-brand-border/40"
              role="img"
        ...
        <div className="grid shrink-0 grid-cols-2 gap-2 sm:gap-3 md:w-[min(100%,20rem)]">
          <div className="rounded-lg bg-white/90 p-3 shadow-sm ring-1 ring-brand-success/10">
            ...
            <p className="mt-0.5 text-xs font-semibold leading-snug text-brand-ink">{withSponsor.label}</p>
          </div>
          <div className="rounded-lg bg-white/90 p-3 shadow-sm ring-1 ring-brand-accent/15">
            <p className="font-heading text-lg font-bold tabular-nums text-brand-accent-dark sm:text-xl">
